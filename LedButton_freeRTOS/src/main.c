...
void buttom_task_handler(void *params);
...
int main(void) {
	...
	xTaskCreate(buttom_task_handler, "BUTTOM_TASK", configMINIMAL_STACK_SIZE, NULL, 1,NULL);
...
void buttom_task_handler(void *params) {
	while (1) {
			if(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13)){
				...
				SEGGER_SYSVIEW_Print("Button not pressed");
			}else{
				// button not pressed on the nucleo board
				button_status_flag=PRESSED;
				SEGGER_SYSVIEW_Print("Button pressed");
			}
	}
}
